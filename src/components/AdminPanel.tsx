...
import { signOut, createUserWithEmailAndPassword, signInWithEmailAndPassword, updateEmail, updatePassword } from "firebase/auth";
import { initializeApp, getApp, getApps } from "firebase/app";
import { getAuth } from "firebase/auth";
import { collection, onSnapshot, query, orderBy, doc, updateDoc, getDoc, getDocs, setDoc, addDoc, deleteDoc, writeBatch, where } from "firebase/firestore";
import { motion, AnimatePresence } from "framer-motion";
import { Store, BarChart3, Search, Package, DollarSign, Clock, LogOut, Box, Plus, Minus, RefreshCw, X, History, MapPin, LayoutDashboard, ClipboardList, Wrench, Shirt, ArrowRight, Edit2, Check, Save, Trash2, FileText, FileSpreadsheet, Users, Menu, Tag, Gift } from "lucide-react";
...
interface Order {
  ...
  date: string;
  items: { id: string; name: string; quantity: number; price: number; category: 'Raw Material' | 'Equipment' | 'Uniform'; itemDiscount?: number }[];
}
...
interface AdminPermissions {
  ...
  discounts: boolean;
  gifts: boolean;
  region_setup: boolean;
  damage_ledger: boolean;
}
...
export default function AdminPanel() {
  ...
  const [isOrdersOnline, setIsOrdersOnline] = useState<boolean>(true);
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'history' | 'franchise' | 'region' | 'inventory' | 'revenue' | 'discounts' | 'gifts' | 'region_setup' | 'damage_ledger' | 'users' | 'admins'>('orders');
  const [dynamicRegions, setDynamicRegions] = useState<Region[]>([]);
  const [damagedStock, setDamagedStock] = useState<any[]>([]);
  const [selectedItemForDamage, setSelectedItemForDamage] = useState<{ order: Order, item: any } | null>(null);
  const [damageQty, setDamageQty] = useState(1);
  const [damageReason, setDamageReason] = useState("");
  const [damageAction, setDamageAction] = useState<'refund' | 'replace'>('refund');
  ...
  const [selectedRegionForNewFranchise, setSelectedRegionForNewFranchise] = useState<string | null>(null);
  const [newFranchise, setNewFranchise] = useState({ name: '', city: '' });
  ...
  const handleUpdateOrder = async () => {
    ...
    const totalAmount = editingOrder.items.reduce((sum, item) => sum + (((item.price || 0) * (item.quantity || 0)) - (item.itemDiscount || 0)), 0);
    ...
    try {
      ...
      if (originalOrder.status !== 'Cancelled') {
        ...
        originalOrder.items.forEach(item => {
          const actualId = item.id.startsWith('GIFT-') ? item.id.replace('GIFT-', '') : item.id;
          itemDiffs[actualId] = (itemDiffs[actualId] || 0) - item.quantity;
        });
      ...
      batch.update(doc(db, "orders", editingOrder.id), cleanUndefined({
  ...
  const addFreeGiftToEditingOrder = () => {
    if (!editingOrder || !discountItemId || discountQty <= 0) return;

    let giftItem;
    if (discountItemId === "Special Custom Gift") {
      giftItem = {
        id: `GIFT-CUSTOM-${Date.now()}`,
        name: `Special Custom Gift (Free Gift)`,
        price: 0,
      ...
      const selectedInvItem = inventory.find(i => i.id === discountItemId);
      if (!selectedInvItem) return;
      giftItem = {
        id: `GIFT-${selectedInvItem.id}`,
        name: `${selectedInvItem.name} (Free Gift)`,
        price: 0,
        quantity: discountQty,
        category: selectedInvItem.type === 'raw' ? 'Raw Material' : selectedInvItem.type === 'equipment' ? 'Equipment' : 'Uniform' as any,
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "regions"), (snapshot) => {
      const regionsData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Region));
      setDynamicRegions(regionsData);
    });
    return () => unsubscribe();
  }, []);

  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "damagedStock"), (snapshot) => {
      const damageData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setDamagedStock(damageData);
    });
    return () => unsubscribe();
  }, []);

  useEffect(() => {
    const unsubscribe = onSnapshot(doc(db, "systemSettings", "ordersControl"), (snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.data();
        if (data && typeof data.online === 'boolean') {
          setIsOrdersOnline(data.online);
        }
      }
    });
    return () => unsubscribe();
  ...
  const handleToggleOrdersControl = async () => {
    setIsUpdatingSystemSettings(true);
    try {
      await setDoc(doc(db, "systemSettings", "ordersControl"), { online: !isOrdersOnline }, { merge: true });
    } catch (error) {
      console.error("Error toggling orders control:", error);
      alert("Failed to toggle system ordering status.");
    } finally {
  ...
  const seedRegions = async () => {
    ...
    try {
      ...
      alert("Regions seeded successfully!");
    } catch (error) {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "adminUsers"), (snapshot) => {
      ...
      } else if (auth.currentUser?.email === "samrarazzaq@gmail.com") {
  ...
  const handleUpdateAdmin = async () => {
    ...
    try {
      await updateDoc(doc(db, "adminUsers", editingAdmin.id), cleanUndefined({
  ...
  const updateStatus = async (orderId: string, status: Order['status'], items: Order['items']) => {
    ...
    if (status === 'Cancelled') {
      ...
      const actualIds = items.map(item => item.id.startsWith('GIFT-') ? item.id.replace('GIFT-', '') : item.id);
      const inventorySnapshots = await Promise.all(
        actualIds.map(id => getDoc(doc(db, "inventory", id)))
      );
      ...
      inventorySnapshots.forEach((invDoc, index) => {
        if (invDoc.exists()) {
          ...
          const invDocRef = doc(db, "inventory", actualIds[index]);
  ...
  const updateInventoryItem = async (itemId: string, updates: Partial<InventoryItem>) => {
    ...
    const invDocRef = doc(db, "inventory", itemId);
    await updateDoc(invDocRef, cleanUndefined(updates));
    setEditingItem(null);
  };
  ...
  const addUser = async () => {
    ...
    const availableRegions = dynamicRegions.length > 0 ? dynamicRegions : regions;
    const region = availableRegions.find(r => r.id === newUser.regionId);
  ...
  const updateUser = async (userId: string, updates: Partial<FranchiseUser>) => {
    try {
      const originalUser = franchiseUsers.find(u => u.id === userId);

      if (originalUser && (updates.email !== originalUser.email || updates.password !== originalUser.password)) {
        const secondaryConfig = {
          apiKey: "AIzaSyAakFasuwjBerQkZu6KVMzH1SE-c_F8qK0",
          authDomain: "wedrink-2bf78.firebaseapp.com",
          projectId: "wedrink-2bf78",
        ...
        try {
          ...
          if (updates.email && updates.email !== originalUser.email) {
            await updateEmail(userCredential.user, updates.email.toLowerCase().trim());
            updates.email = updates.email.toLowerCase().trim();
          }
          if (updates.password && updates.password !== originalUser.password) {
            await updatePassword(userCredential.user, updates.password);
          }

          await signOut(secondaryAuth);
        } catch (authError: any) {
          console.error("Auth update error:", authError);
          alert("Failed to update credentials in Auth. Ensure the original password was correct. Error: " + authError.message);
          return;
        }
      }

      await updateDoc(doc(db, "franchiseUsers", userId), cleanUndefined(updates));
      setEditingUser(null);
    } catch (error) {
  ...
  const handleMarkDamage = async () => {
    if (!selectedItemForDamage || !damageReason || damageQty <= 0) return;
    setIsProcessingDamage(true);
    try {
      const { order, item } = selectedItemForDamage;
      const unitPrice = item.price - (item.itemDiscount ? item.itemDiscount / item.quantity : 0);
      const totalDamageAmount = unitPrice * damageQty;

      // 1. Log Damage
      await addDoc(collection(db, "damagedStock"), {
        orderId: order.id,
      ...
      if (damageAction === 'refund') {
        const usersRef = collection(db, "franchiseUsers");
        const q = query(usersRef, where("franchiseId", "==", order.franchiseId));
        const userSnapshot = await getDocs(q);

        if (!userSnapshot.empty) {
          const userDoc = userSnapshot.docs[0];
          const currentBalance = userDoc.data().balance || 0;
          await updateDoc(doc(db, "franchiseUsers", userDoc.id), {
            balance: currentBalance + totalDamageAmount
          });
        }

        const updatedItems = order.items.map(i => {
          if (i.id === item.id) {
            return { ...i, quantity: i.quantity - damageQty };
          }
          return i;
        }).filter(i => i.quantity > 0);

        const newTotal = updatedItems.reduce((sum, i) => sum + (i.price * i.quantity), 0);
        const discountVal = (newTotal * (order.discountPercent || 0)) / 100;
        const finalVal = Math.max(0, newTotal - discountVal - (order.balanceAdjustment || 0));
        ...
        if (invItem) {
          ...
          await updateDoc(invDocRef, {
            quantity: Math.max(0, invItem.quantity - damageQty)
          });
        }
      }

      alert(`Successfully processed damage as ${damageAction.toUpperCase()}`);
  ...
  const exportOrderToPDF = (order: Order) => {
    ...
    // Items Table
    const tableColumn = ["Item", "Category", "Quantity", "Price", "Discount", "Total"];
    const tableRows = order.items.map(item => [
      item.name,
      item.category,
      item.quantity.toString(),
      `Rs. ${item.price.toLocaleString()}`,
      item.itemDiscount ? `Rs. ${item.itemDiscount.toLocaleString()}` : '-',
      `Rs. ${((item.price * item.quantity) - (item.itemDiscount || 0)).toLocaleString()}`
    ]);
  ...
  const exportOrderToExcel = (order: Order) => {
    const worksheetData: any[][] = [
      ...
      ["Status", order.status],
      [],
      ["Item", "Category", "Quantity", "Price", "Discount", "Total"],
      ...order.items.map(item => [
        ...
        item.price,
        item.itemDiscount || 0,
        (item.price * item.quantity) - (item.itemDiscount || 0)
      ]),
      []
    ];
  ...
  const groupedOrdersByDate = useMemo(() => {
    ...
    orders.forEach(order => {
      ...
      // Filter by Search
      if (search && !order.franchiseName.toLowerCase().includes(search.toLowerCase()) && !order.regionName.toLowerCase().includes(search.toLowerCase())) return;

      // Filter by Date
      if (ledgerDateFilter) {
        const filterDate = new Date(ledgerDateFilter);
        if (orderDate.toLocaleDateString() !== filterDate.toLocaleDateString()) return;
      }

      // Filter by Month
      if (ledgerMonthFilter) {
        const [year, month] = ledgerMonthFilter.split('-');
        if (orderDate.getFullYear() !== parseInt(year) || (orderDate.getMonth() + 1) !== parseInt(month)) return;
      }
  ...
  }, [orders, ledgerDateFilter, ledgerMonthFilter, search]);
    ...
    <div className="flex flex-col lg:flex-row bg-slate-50 min-h-screen selection:bg-teal-100 selection:text-teal-900 font-sans overflow-hidden">
        ...
        <div className="p-8">
          ...
          <nav className="space-y-2">
            {[
              ...
              { id: 'discounts', icon: Tag, label: 'Discount Ledger', permission: 'discounts' },
              { id: 'gifts', icon: Gift, label: 'Gift Ledger', permission: 'gifts' },
              { id: 'damage_ledger', icon: Trash2, label: 'Damage Ledger', permission: 'damage_ledger' },
              { id: 'region_setup', icon: MapPin, label: 'Region Setup', permission: 'region_setup' },
            ...
            ].filter(tab => currentAdmin?.role === 'super_admin' ? true : currentAdmin?.permissions[tab.permission as keyof AdminPermissions]).map(tab => (
              <button
                key={tab.id}
                onClick={() => {
                ...
                className={`w-full px-4 py-3.5 rounded-2xl font-bold flex items-center gap-3 transition-all group ${activeTab === tab.id
      ...
      <main className="flex-1 p-4 md:p-10 overflow-y-auto max-h-screen bg-gradient-to-br from-slate-50 via-white to-teal-50/30">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 md:mb-10">
          ...
          {activeTab === 'orders' && (
            <button
              onClick={handleToggleOrdersControl}
              disabled={isUpdatingSystemSettings}
              className={`flex items-center gap-3 bg-white px-4 py-2 rounded-2xl border shadow-sm transition-all ${isOrdersOnline
                  ? 'text-green-600 border-green-100 hover:border-green-300 hover:bg-green-50/10'
                  : 'text-red-600 border-red-100 hover:border-red-300 hover:bg-red-50/10'
                }`}
              title="Click to toggle Ordering System Online/Offline"
            >
              <div className={`w-2 h-2 rounded-full ${isOrdersOnline ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`} />
              <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest">
                Ordering: {isOrdersOnline ? 'Online' : 'Offline'}
              </span>
            </button>
          )}
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                  ...
                  <div
                    ...
                    className={`bg-white p-6 md:p-8 rounded-2xl md:rounded-[2.5rem] shadow-sm border flex items-center gap-4 md:gap-6 group hover:shadow-xl hover:shadow-${stat.color}-900/5 transition-all cursor-pointer ${(stat.id === 'total' && orderFilter === 'all') || (stat.id === 'pending' && orderFilter === 'pending')
                    ...
                    <div className="flex flex-wrap items-center justify-between md:justify-end gap-4 md:gap-6 w-full md:w-auto">
                      <div className="flex flex-col items-start md:items-end">
                        ...
                        <select
                          ...
                          className={`appearance-none px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl font-bold text-xs md:text-sm focus:outline-none transition-all cursor-pointer border-2 ${order.status === 'Pending' ? 'bg-yellow-50 text-yellow-700 border-yellow-100 hover:border-yellow-200' :
          ...
          {activeTab === 'history' && (
            ...
            >
              {/* History Filters */}
              <div className="bg-white/80 backdrop-blur-md p-4 md:p-6 rounded-2xl md:rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-900/5 flex flex-col lg:flex-row items-stretch lg:items-center gap-4 md:gap-6 sticky top-0 z-30">
                <div className="flex-1 min-w-0 relative">
                  ...
                  <input
                    ...
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    className="w-full pl-12 pr-4 py-3 bg-slate-50 rounded-xl border border-slate-100 focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 outline-none transition-all text-sm font-bold text-slate-700 placeholder:text-slate-400"
                  />
                </div>
                <div className="flex flex-wrap items-end gap-3 md:gap-4">
                  <div className="flex-1 flex flex-col min-w-[120px]">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1">Specific Date</label>
                    <input
                      type="date"
                      value={ledgerDateFilter}
                      onChange={(e) => {
                        setLedgerDateFilter(e.target.value);
                        if (e.target.value) setLedgerMonthFilter("");
                      }}
                      className="w-full px-4 py-2.5 bg-slate-50 rounded-xl border border-slate-100 focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 outline-none transition-all text-xs font-bold text-slate-700"
                    />
                  </div>
                  <div className="flex-1 flex flex-col min-w-[120px]">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1">Month</label>
                    <input
                      type="month"
                      value={ledgerMonthFilter}
                      onChange={(e) => {
                        setLedgerMonthFilter(e.target.value);
                        if (e.target.value) setLedgerDateFilter("");
                      }}
                      className="w-full px-4 py-2.5 bg-slate-50 rounded-xl border border-slate-100 focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 outline-none transition-all text-xs font-bold text-slate-700"
                    />
                  </div>
                  {(ledgerDateFilter || ledgerMonthFilter || search) && (
                    <button
                      onClick={() => {
                        setLedgerDateFilter("");
                      ...
                      className="p-3 bg-red-50 text-red-600 rounded-xl hover:bg-red-100 transition-colors shadow-sm shadow-red-100"
                      title="Clear Filters"
                    >
                        ...
                        <div key={order.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:border-teal-200 hover:shadow-xl hover:shadow-teal-900/5 transition-all group">
                          ...
                          <div className="flex items-center gap-8 w-full md:w-auto">
                            <div className="flex flex-col items-end">
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Status</p>
                              <span className={`px-4 py-1.5 rounded-xl text-xs font-black uppercase tracking-widest ${order.status === 'Pending' ? 'bg-yellow-100 text-yellow-700' :
                            ...
                            <div className="ml-auto md:ml-0 flex items-center gap-2">
                              ...
                              <button
                                onClick={() => setSelectedOrder(order)}
                                className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-teal-600 hover:text-white transition-all shadow-sm"
                              >
                      ...
                      {orders.filter(o => o.status !== 'Cancelled' && o.status !== 'Pending').slice().sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((order) => (
                        <tr key={order.id} className="hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => setSelectedOrder(order)}>
                          ...
                          <td className="px-8 py-6 text-center">
                            <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${order.status === 'Pending' ? 'bg-yellow-50 text-yellow-700' :
          ...
          {activeTab === 'discounts' && (
            ...
            >
              <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="flex items-center gap-4 md:gap-6">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-50 rounded-2xl md:rounded-3xl flex items-center justify-center text-blue-600">
                    <Tag className="w-6 h-6 md:w-8 md:h-8" />
                  </div>
                  <div>
                    <h2 className="text-xl md:text-3xl font-black text-slate-900 tracking-tight">Discount Ledger</h2>
                    <p className="text-xs md:text-sm text-slate-500 font-medium mt-1">Detailed breakdown of all discounts given</p>
                  </div>
                </div>
                <div className="text-left md:text-right flex flex-col items-start md:items-end gap-3 w-full md:w-auto">
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Discount Given</p>
                    <p className="text-2xl md:text-4xl font-black text-blue-600">
                      Rs. {orders.filter(o => o.status !== 'Cancelled').reduce((sum, order) => {
                        const itemDiscountTotal = order.items.reduce((acc, item) => acc + (item.itemDiscount || 0), 0);
                        return sum + (order.discount || 0) + itemDiscountTotal;
                      }, 0).toLocaleString()}
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-100">
                        ...
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Order ID</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Franchise</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Order Discount</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Item Discounts</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Total Discount</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {orders
                        ...
                        .filter(o => {
                          const itemDiscountTotal = o.items.reduce((acc, item) => acc + (item.itemDiscount || 0), 0);
                          return (o.discount || 0) > 0 || itemDiscountTotal > 0;
                        })
                        .slice()
                        .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
                        .map((order) => {
                          ...
                          return (
                            <tr key={order.id} className="hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => setSelectedOrder(order)}>
                              ...
                              <td className="px-8 py-6">
                                <p className="font-bold text-slate-900">{order.franchiseName}</p>
                                <p className="text-sm text-slate-500">{order.regionName}</p>
                              </td>
                              <td className="px-8 py-6 text-right">
                                <p className="font-bold text-slate-600">Rs. {(order.discount || 0).toLocaleString()}</p>
                                {order.discountPercent ? <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{order.discountPercent}% OFF</p> : null}
                              </td>
                              <td className="px-8 py-6 text-right">
                                <p className="font-bold text-slate-600">Rs. {itemDiscountTotal.toLocaleString()}</p>
                                {order.items.filter(i => (i.itemDiscount || 0) > 0).map((i, idx) => (
                                  <p key={`${i.id}-${idx}`} className="text-[10px] text-teal-600 font-bold tracking-widest mt-1">
                                    {i.name} <span className="text-slate-400">(-{i.itemDiscount})</span>
                                  </p>
                                ))}
                              </td>
                              <td className="px-8 py-6 text-right">
                                <p className="font-black text-blue-600">Rs. {totalDiscount.toLocaleString()}</p>
                              </td>
                            </tr>
                          );
                        })}
                ...
                {orders.filter(o => o.status !== 'Cancelled' && ((o.discount || 0) > 0 || o.items.some(i => (i.itemDiscount || 0) > 0))).length === 0 && (
                  <div className="p-20 text-center">
                    <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6">
                      <Tag className="w-10 h-10 text-slate-300" />
                    </div>
                    <h3 className="text-xl font-black text-slate-900">No Discounts Given</h3>
                    <p className="text-slate-500 mt-2">Discounts will appear here once applied to orders.</p>
                  </div>
                )}
              </div>
            </motion.div>
          ...
          {activeTab === 'gifts' && (
            ...
            >
              <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="flex items-center gap-4 md:gap-6">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-pink-50 rounded-2xl md:rounded-3xl flex items-center justify-center text-pink-600">
                    <Gift className="w-6 h-6 md:w-8 md:h-8" />
                  </div>
                  <div>
                    <h2 className="text-xl md:text-3xl font-black text-slate-900 tracking-tight">Gift Ledger</h2>
                    <p className="text-xs md:text-sm text-slate-500 font-medium mt-1">Detailed record of all complimentary items given</p>
                  </div>
                </div>
                <div className="text-left md:text-right flex flex-col items-start md:items-end gap-3 w-full md:w-auto">
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Gifts Given</p>
                    <p className="text-2xl md:text-4xl font-black text-pink-600">
                      {orders.filter(o => o.status !== 'Cancelled').reduce((sum, order) => {
                        return sum + order.items.filter(i => i.id.startsWith('GIFT-')).reduce((acc, i) => acc + i.quantity, 0);
                      }, 0).toLocaleString()} Items
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-100">
                        ...
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Order ID</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Franchise</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Gift Items</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Total Qty</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {orders
                        .filter(o => o.status !== 'Cancelled')
                        .filter(o => o.items.some(i => i.id.startsWith('GIFT-')))
                        .slice()
                        .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
                        .map((order) => {
                          const giftItems = order.items.filter(i => i.id.startsWith('GIFT-'));
                          const totalGiftQty = giftItems.reduce((acc, i) => acc + i.quantity, 0);
                          return (
                            <tr key={order.id} className="hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => setSelectedOrder(order)}>
                              <td className="px-8 py-6">
                                <p className="font-bold text-slate-900">{new Date(order.date).toLocaleDateString()}</p>
                              ...
                              <td className="px-8 py-6 text-right">
                                {giftItems.map((i, idx) => (
                                  <p key={`${i.id}-${idx}`} className="text-[10px] text-pink-600 font-bold tracking-widest mt-1">
                                    {i.name.replace(' (Free Gift)', '')} <span className="text-slate-400">(x{i.quantity})</span>
                                  </p>
                                ))}
                              </td>
                              <td className="px-8 py-6 text-right">
                                <p className="font-black text-pink-600">{totalGiftQty.toLocaleString()}</p>
                              </td>
                            </tr>
                          );
                        })}
                ...
                {orders.filter(o => o.status !== 'Cancelled' && o.items.some(i => i.id.startsWith('GIFT-'))).length === 0 && (
                  <div className="p-20 text-center">
                    <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6">
                      <Gift className="w-10 h-10 text-slate-300" />
                    </div>
                    <h3 className="text-xl font-black text-slate-900">No Gifts Given</h3>
                    <p className="text-slate-500 mt-2">Free gifts will appear here once added to orders.</p>
                  </div>
                )}
              </div>
            </motion.div>
          ...
          {activeTab === 'damage_ledger' && (
            ...
            >
              <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="flex items-center gap-4 md:gap-6">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-red-50 rounded-2xl md:rounded-3xl flex items-center justify-center text-red-600">
                    <Trash2 className="w-6 h-6 md:w-8 md:h-8" />
                  </div>
                  <div>
                    <h2 className="text-xl md:text-3xl font-black text-slate-900 tracking-tight">Damage Ledger</h2>
                    <p className="text-xs md:text-sm text-slate-500 font-medium mt-1">Record of all damaged, returned, or replaced items</p>
                  </div>
                </div>
                <div className="text-left md:text-right flex flex-col items-start md:items-end gap-3 w-full md:w-auto">
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Damage Value</p>
                    <p className="text-2xl md:text-4xl font-black text-red-600">
                      Rs. {damagedStock.reduce((sum, item) => sum + (item.totalAmount || 0), 0).toLocaleString()}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-100">
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Date</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Franchise / Order</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Item / Reason</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Qty</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Action Taken</th>
                        <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Amount</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                        ...
                        .map((record) => (
                          <tr key={record.id} className="hover:bg-slate-50/50 transition-colors">
                            <td className="px-8 py-6">
                              ...
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{new Date(record.date).toLocaleTimeString()}</p>
                            </td>
                            <td className="px-8 py-6">
                              <p className="font-bold text-slate-900">{record.franchiseName}</p>
                              <p className="text-xs font-mono text-slate-400">#{record.orderId.slice(-8)}</p>
                            </td>
                            <td className="px-8 py-6">
                              <p className="font-bold text-slate-900">{record.itemName}</p>
                              <p className="text-xs text-red-500 font-medium italic">"{record.reason}"</p>
                            </td>
                            <td className="px-8 py-6 text-center font-black text-slate-700">{record.quantity}</td>
                            <td className="px-8 py-6 text-right">
                              <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${record.action === 'refund' ? 'bg-teal-100 text-teal-700' : 'bg-blue-100 text-blue-700'
                                }`}>
                                {record.action === 'refund' ? 'Refunded' : 'Replaced'}
                              </span>
                            </td>
                            <td className="px-8 py-6 text-right">
                              <p className="font-black text-slate-900">Rs. {(record.totalAmount || 0).toLocaleString()}</p>
                            </td>
                          </tr>
                        ))}
                    </tbody>
                ...
                {damagedStock.length === 0 && (
                  <div className="p-20 text-center">
                    ...
                    <h3 className="text-xl font-black text-slate-900">No Damage Records</h3>
                    <p className="text-slate-500 mt-2">Any items marked as damaged or replaced will appear here.</p>
                  </div>
                )}
              </div>
            </motion.div>
          ...
          {activeTab === 'region_setup' && (
            ...
            >
              <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="flex items-center gap-4 md:gap-6">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-teal-50 rounded-2xl md:rounded-3xl flex items-center justify-center text-teal-600">
                    <MapPin className="w-6 h-6 md:w-8 md:h-8" />
                  </div>
                  <div>
                    <h2 className="text-xl md:text-3xl font-black text-slate-900 tracking-tight">Region & Franchise Setup</h2>
                    <p className="text-xs md:text-sm text-slate-500 font-medium mt-1">Manage dynamic regions and their franchises</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 space-y-6">
                  <div className="flex justify-between items-center">
                    <h3 className="text-xl font-black text-slate-900">Regions</h3>
                    <button
                  ...
                  <div className="space-y-3">
                    {(dynamicRegions.length > 0 ? dynamicRegions : regions).map(region => (
                      <div
                        key={region.id}
                        onClick={() => setSelectedRegionId(region.id)}
                        className={`p-4 rounded-2xl border cursor-pointer transition-all flex justify-between items-center group ${selectedRegionId === region.id ? 'bg-teal-600 border-teal-600 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-900 hover:border-teal-200'
                          }`}
                      >
                        <div>
                          <p className="font-bold">{region.name}</p>
                          <p className={`text-[10px] font-bold uppercase tracking-widest ${selectedRegionId === region.id ? 'text-teal-100' : 'text-slate-400'}`}>
                            {region.franchises.length} Franchises
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setSelectedRegionForNewFranchise(region.id);
                              setIsAddingFranchise(true);
                            }}
                            className={`p-2 rounded-lg transition-all flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest ${selectedRegionId === region.id ? 'bg-white/20 text-white' : 'bg-white text-teal-600 opacity-0 group-hover:opacity-100 shadow-sm'
                              }`}
                          >
                ...
                {/* Franchises List for Selected Region */}
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 space-y-6">
                  {selectedRegionId ? (
                    <>
                      <div className="flex justify-between items-center">
                        <div>
                          <h3 className="text-xl font-black text-slate-900">Franchises</h3>
                          <p className="text-xs font-bold text-teal-600 uppercase tracking-widest mt-1">
                            In {(dynamicRegions.length > 0 ? dynamicRegions : regions).find(r => r.id === selectedRegionId)?.name}
                          </p>
                        </div>
                      </div>

                      <div className="space-y-3">
                        {(dynamicRegions.length > 0 ? dynamicRegions : regions).find(r => r.id === selectedRegionId)?.franchises.map(franchise => (
                          <div key={franchise.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex justify-between items-center group hover:border-red-100 transition-all">
                            <div>
                              <p className="font-bold text-slate-900">{franchise.name}</p>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{franchise.city}</p>
                            </div>
                            <button
                              onClick={async () => {
                                if (!confirm(`Are you sure you want to remove "${franchise.name}"?`)) return;
                                const region = dynamicRegions.find(r => r.id === selectedRegionId);
                                if (!region) {
                                  alert("Cannot delete from static data. Please 'Seed' first.");
                                  return;
                                }
                                const updatedFranchises = region.franchises.filter(f => f.id !== franchise.id);
                                await updateDoc(doc(db, "regions", selectedRegionId), {
                        ...
                        {((dynamicRegions.length > 0 ? dynamicRegions : regions).find(r => r.id === selectedRegionId)?.franchises.length === 0) && (
                          <div className="py-20 text-center">
                            <Store className="w-10 h-10 text-slate-200 mx-auto mb-4" />
                            <p className="text-slate-400 font-bold">No franchises found in this region.</p>
                          </div>
                        )}
                      </div>
                    </>
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center p-10">
                      ...
                      <h3 className="text-xl font-black text-slate-900">Select a Region</h3>
                      <p className="text-slate-500 mt-2">Click on a region from the left to manage its franchises.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              {isAddingRegion && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                  <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
                    <h3 className="text-2xl font-black text-slate-900 mb-6">Add New Region</h3>
                    <input
                      type="text"
                      placeholder="Region Name (e.g. Peshawar)"
                      value={newRegionName}
                      onChange={(e) => setNewRegionName(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-4 font-bold text-slate-900 mb-6 focus:outline-none focus:ring-2 focus:ring-teal-500"
                    />
                    <div className="flex gap-3">
                      <button onClick={() => setIsAddingRegion(false)} className="flex-1 py-4 font-bold text-slate-400 hover:text-slate-600 transition-colors">Cancel</button>
                      <button
                        onClick={async () => {
                          if (!newRegionName) return;
                          const regionId = `reg-${newRegionName.toLowerCase().replace(/\s+/g, '-')}`;
                        ...
                        className="flex-2 bg-teal-600 text-white px-8 py-4 rounded-2xl font-bold hover:bg-teal-700 transition-all shadow-lg shadow-teal-100"
                      >
                        Save Region
                      </button>
                    </div>
              ...
              {isAddingFranchise && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                  <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
                    <h3 className="text-2xl font-black text-slate-900 mb-2">Add New Branch</h3>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6">
                      In Region: {dynamicRegions.find(r => r.id === selectedRegionForNewFranchise)?.name}
                    </p>
                    <div className="space-y-4 mb-8">
                      <input
                        type="text"
                        placeholder="Branch Name (e.g. G-9 Markaz)"
                        value={newFranchise.name}
                        onChange={(e) => setNewFranchise({ ...newFranchise, name: e.target.value })}
                        className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-4 font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500"
                      />
                      <input
                        type="text"
                        placeholder="City"
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => setIsAddingFranchise(false)} className="flex-1 py-4 font-bold text-slate-400 hover:text-slate-600 transition-colors">Cancel</button>
                      <button
                        onClick={async () => {
                          if (!newFranchise.name || !newFranchise.city || !selectedRegionForNewFranchise) return;
                          const region = dynamicRegions.find(r => r.id === selectedRegionForNewFranchise);
                          if (!region) return;

                          const franchiseId = `f-${region.id.replace('reg-', '')}-${Date.now()}`;
                          const updatedFranchises = [...region.franchises, { ...newFranchise, id: franchiseId }];

                          await updateDoc(doc(db, "regions", selectedRegionForNewFranchise), {
                            franchises: updatedFranchises
                          });

                          setNewFranchise({ name: '', city: '' });
                      ...
                      <div className="flex overflow-x-auto gap-2 bg-slate-50 p-1.5 rounded-2xl border border-slate-100 shadow-sm no-scrollbar">
                        {(['all', 'Raw Material', 'Equipment', 'Uniform'] as const).map(cat => (
                          <button
                            ...
                            className={`px-4 py-2 rounded-xl font-bold capitalize transition-all text-[10px] md:text-xs whitespace-nowrap ${franchiseCategoryFilter === cat ? 'bg-teal-600 text-white shadow-md shadow-teal-100' : 'text-slate-500 hover:bg-slate-100'
                              }`}
                          >
                            {cat}
                          </button>
                      ...
                      <div className="flex gap-2 bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm">
                        {(['all', 'raw', 'equipment', 'uniform'] as const).map(cat => (
                          <button
                            ...
                            className={`px-6 py-2.5 rounded-xl font-bold capitalize transition-all text-sm ${inventoryCategory === cat ? 'bg-teal-600 text-white shadow-lg shadow-teal-100' : 'text-slate-500 hover:bg-slate-50'
                              }`}
                          >
                            {cat}
                          </button>
                            ...
                            <div className="relative z-10">
                              <div className="flex justify-between items-start mb-6 md:mb-8">
                                <div className="flex items-center gap-4 md:gap-5">
                                  <div className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center transition-all group-hover:scale-110 ${item.type === 'raw' ? 'bg-teal-50 text-teal-600' :
                                ...
                                <div>
                                  ...
                                  <div className="flex items-baseline gap-2">
                                    <input
                                      ...
                                      className={`w-16 md:w-24 bg-transparent text-2xl md:text-3xl font-black focus:outline-none focus:text-teal-600 transition-colors ${item.quantity < 10 ? 'text-red-600' : 'text-slate-900'
                                        }`}
                                    />
                                              ...
                                              {['Raw Material', 'Equipment', 'Uniform'].filter(cat => franchiseCategoryFilter === 'all' || cat === franchiseCategoryFilter).map(cat => (
                                                dayData.categories[cat] > 0 && (
                                                  <span key={cat} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                                                    <div className={`w-1.5 h-1.5 rounded-full ${cat === 'Raw Material' ? 'bg-teal-500' :
                                            ...
                                            {['Raw Material', 'Equipment', 'Uniform'].filter(cat => franchiseCategoryFilter === 'all' || cat === franchiseCategoryFilter).map(cat => (
                                              Object.keys(dayData.itemSummary[cat]).length > 0 && (
                                                <div key={cat} className="space-y-4">
                                                  <div className="flex items-center gap-3">
                                                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${cat === 'Raw Material' ? 'bg-teal-100 text-teal-600' :
                                ...
                                <div className="flex flex-wrap gap-2 md:gap-3 mt-2">
                                  {['Raw Material', 'Equipment', 'Uniform'].map(cat => (
                                    item.categories[cat] > 0 && (
                                      <span key={cat} className={`text-[8px] md:text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg border ${franchiseCategoryFilter === cat
              ...
              <div className="p-6 md:p-8 space-y-4 md:space-y-6">
                ...
                <div>
                  ...
                  >
                    ...
                    {(dynamicRegions.length > 0 ? dynamicRegions : regions).map(r => (
                ...
                {newUser.regionId && (
                  <div>
                    ...
                    >
                      ...
                      {(dynamicRegions.length > 0 ? dynamicRegions : regions).find(r => r.id === newUser.regionId)?.franchises.map(f => (
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {editingOrder.items.map((item) => (
                        <div key={item.id} className="flex flex-col gap-2 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                          ...
                          <div className="flex items-center gap-3 pt-2 border-t border-slate-200/50 mt-1">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest w-24">Item Discount</span>
                            <div className="flex items-center flex-1 bg-white border border-slate-200 rounded-lg overflow-hidden">
                              <span className="px-3 text-xs font-bold text-slate-400 bg-slate-50 border-r border-slate-200">Rs.</span>
                              <input
                                type="number"
                                placeholder="0"
                                ...
                                onChange={(e) => {
                                  ...
                                  setEditingOrder({
                                    ...editingOrder,
                                    items: editingOrder.items.map(i => i.id === item.id ? { ...i, itemDiscount: discount } : i)
                                  });
                                }}
                                className="w-full px-3 py-1.5 text-xs font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500"
                              />
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="bg-slate-900 rounded-3xl p-6 text-white space-y-6">
                    ...
                    <div className="pt-6 border-t border-white/10 space-y-3">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold">Rs. {editingOrder.items.reduce((sum, i) => sum + (((i.price || 0) * (i.quantity || 1)) - (i.itemDiscount || 0)), 0).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-400 font-bold uppercase tracking-widest">Discount</span>
                        <span className="font-bold text-green-400">- Rs. {((editingOrder.items.reduce((sum, i) => sum + (((i.price || 0) * (i.quantity || 1)) - (i.itemDiscount || 0)), 0) * (editingOrder.discountPercent || 0)) / 100).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="pt-3 border-t border-white/10 flex justify-between items-end">
                        ...
                        <span className="text-3xl font-black text-teal-400">
                          Rs. {(
                            editingOrder.items.reduce((sum, i) => sum + (((i.price || 0) * (i.quantity || 1)) - (i.itemDiscount || 0)), 0) -
                            (editingOrder.items.reduce((sum, i) => sum + (((i.price || 0) * (i.quantity || 1)) - (i.itemDiscount || 0)), 0) * (editingOrder.discountPercent || 0) / 100) -
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-8 space-y-6">
                ...
                <button
                  onClick={() => updateUser(editingUser.id, { email: editingUser.email, password: editingUser.password, balance: editingUser.balance, discountPercent: editingUser.discountPercent })}
            ...
            <div className="p-4 md:p-8 overflow-y-auto space-y-8">
                        ...
                        <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-right">
                            {selectedOrder.status === 'Delivered' && (
                              <button
                                onClick={() => setSelectedItemForDamage({ order: selectedOrder, item })}
                                className="p-2 text-red-400 hover:bg-red-50 rounded-lg transition-all"
                                title="Mark Damage/Return"
                              >
                                <Trash2 className="w-4 h-4" />
                              </button>
                            )}
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-teal-600 p-6 md:p-8 rounded-2xl md:rounded-[2.5rem] text-white shadow-xl shadow-teal-100 gap-4">
                  ...
                  <div className={`px-4 md:px-6 py-2 rounded-xl md:rounded-2xl font-black uppercase tracking-widest text-[10px] md:text-sm ${selectedOrder.status === 'Delivered' ? 'bg-white/20 text-white' : 'bg-white text-teal-600'
                    }`}>
        ...
        {selectedItemForDamage && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-white rounded-[3rem] p-8 w-full max-w-md shadow-2xl space-y-8"
            >
              <div>
                <h3 className="text-2xl font-black text-slate-900">Mark Damaged Stock</h3>
                <p className="text-sm font-medium text-slate-500 mt-2">Item: {selectedItemForDamage.item.name}</p>
              </div>

              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4 bg-slate-50 p-2 rounded-2xl border border-slate-100">
                  <button
                    onClick={() => setDamageAction('refund')}
                    className={`py-3 rounded-xl font-bold transition-all ${damageAction === 'refund' ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    Refund Balance
                  </button>
                  <button
                    onClick={() => setDamageAction('replace')}
                    className={`py-3 rounded-xl font-bold transition-all ${damageAction === 'replace' ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    Replace Item
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">Quantity (Max: {selectedItemForDamage.item.quantity})</label>
                    <input
                      type="number"
                      max={selectedItemForDamage.item.quantity}
                      min={1}
                      value={damageQty}
                      onChange={(e) => setDamageQty(Math.min(selectedItemForDamage.item.quantity, Math.max(1, parseInt(e.target.value) || 1)))}
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-4 font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">Reason for Damage/Return</label>
                    <textarea
                      placeholder="e.g. Expired, Broken packaging, etc."
                      value={damageReason}
                      onChange={(e) => setDamageReason(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-4 font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500 min-h-[100px]"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                <button
                  onClick={() => setSelectedItemForDamage(null)}
                ...
                <button
                  ...
                  className="flex-2 bg-red-600 text-white px-8 py-4 rounded-2xl font-black hover:bg-red-700 transition-all shadow-lg shadow-red-100 flex items-center justify-center gap-2"
                >
                  {isProcessingDamage ? <RefreshCw className="w-5 h-5 animate-spin" /> : 'Confirm Action'}
                </button>
              </div>
            </motion.div>
          </div>
