...
export default function FranchisePortal() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      if (!snapshot.empty) {
        ...
        const region = availableRegions.find(r => r.id === userData.regionId);
    ...
    return () => unsubscribe();
  }, [auth.currentUser, step, dynamicRegions]);
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!isOrdersOnline) {
      setOrderError("Ordering is currently closed by the administrator.");
      alert("Ordering system is offline. You cannot place orders at this time.");
      return;
    }
          ...
          <AnimatePresence mode="wait">
            {showHistory ? (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-tr from-red-500/10 via-amber-500/5 to-rose-500/10 rounded-[3rem] blur-2xl -z-10" />

                {/* Main Card */}
                <div className="bg-white/80 backdrop-blur-2xl rounded-[3rem] shadow-2xl border border-white/40 overflow-hidden relative">

                  {/* Neon Indicator Strip */}
                  <div className="h-2.5 w-full bg-gradient-to-r from-red-500 via-rose-500 to-amber-500 animate-pulse" />

                  <div className="p-8 md:p-14 space-y-8 text-center relative z-10">

                    {/* Glowing Logo Container */}
                    <div className="relative w-28 h-28 mx-auto flex items-center justify-center">
                      <div className="absolute inset-0 rounded-3xl bg-red-500/5 rotate-6 animate-pulse" />
                      <div className="absolute inset-0 rounded-3xl bg-red-100/40 -rotate-3 animate-ping" style={{ animationDuration: '3s' }} />
                      <div className="relative w-24 h-24 bg-white rounded-3xl border border-slate-100 shadow-xl flex items-center justify-center p-3 group overflow-hidden">
                        ...
                        <img src="/Logo.png" alt="WEDRINK Logo" className="w-full h-full object-contain relative z-10 animate-bounce" style={{ animationDuration: '4s' }} />
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight leading-none">
                        Ordering Portal Suspended
                      </h2>
                      <div className="inline-flex items-center gap-2 px-3.5 py-1 bg-red-50/80 border border-red-100/60 rounded-full text-red-600 font-extrabold text-[10px] tracking-wider uppercase">
                        <span className="w-2 h-2 rounded-full bg-red-500 animate-ping" />
                        Awaiting Activation
                      </div>
                    </div>

                    {/* High-Contrast Alert Notice Box */}
                    <div className="bg-red-50/70 border border-red-200 rounded-[2rem] p-8 md:p-10 text-left space-y-6 shadow-sm relative overflow-hidden">
                      ...
                      <div className="relative z-10 space-y-6">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex gap-4 items-start">
                            <div className="w-6 h-6 rounded-full bg-red-100/80 text-red-600 flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold font-mono">1</div>
                            <p className="text-sm md:text-base text-slate-700 leading-relaxed font-semibold">
                              The software trial period has ended and the <span className="text-slate-900 font-extrabold underline decoration-red-300">activation payment from WeDrink management </span> is currently pending.
                            </p>
                          </div>

                          {/* Point 2 */}
                          <div className="flex gap-4 items-start">
                            <div className="w-6 h-6 rounded-full bg-red-100/80 text-red-600 flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold font-mono">2</div>
                            <p className="text-sm md:text-base text-slate-700 leading-relaxed font-semibold">
                              Consequently, the franchise ordering service has been deactivated
                            </p>
                          </div>

                          {/* Point 3 */}
                          <div className="flex gap-4 items-start">
                            <div className="w-6 h-6 rounded-full bg-teal-100/80 text-teal-700 flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold font-mono">3</div>
                            <p className="text-sm md:text-base text-slate-700 leading-relaxed font-semibold">
                              The system will automatically return to normal as soon as the management completes the payment.
                            </p>
                          </div>
                        ...
                        {/* Footer Subtext */}
                        <div className="bg-white/60 border border-red-100/50 rounded-2xl p-4 flex items-center gap-3 text-xs md:text-sm text-slate-500 font-medium">
                          <span className="w-2 h-2 rounded-full bg-red-400 animate-pulse shrink-0" />
                          <p>
                            We apologize for the inconvenience. For updates, please contact the <span className="text-slate-900 font-extrabold underline">Head Office</span> directly.
                          </p>
                        </div>
                      </div>
                    </div>

                    {/* Actions and Support links */}
                    <div className="flex flex-col sm:flex-row gap-3 justify-center items-center pt-2">
                      <button
                        onClick={() => setShowHistory(true)}
                        className="w-full sm:w-auto px-8 py-4 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl font-bold text-sm transition-all flex items-center justify-center gap-2 hover:shadow-xl shadow-slate-900/10 active:scale-[0.98] group"
                      >
                        <History className="w-4 h-4 text-teal-400 group-hover:rotate-12 transition-transform" />
                          ...
                          >
                            {selectedRegion.franchises.map((franchise) => (
                              <motion.button
                                ...
                                className={`group relative p-8 rounded-[2.5rem] border-2 transition-all text-left overflow-hidden ${selectedFranchise?.id === franchise.id
                              ...
                              >
                                <div className="relative z-10 space-y-4">
                                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all overflow-hidden ${selectedFranchise?.id === franchise.id ? 'bg-white shadow-lg shadow-teal-600/20' : 'bg-slate-100 group-hover:bg-teal-100'
                                    }`}>
                                    <img src="/Logo.png" alt="WEDRINK" className="w-10 h-10 object-contain" referrerPolicy="no-referrer" />
                                  </div>
                                  <div>
                                    <h3 className={`text-2xl font-black transition-colors ${selectedFranchise?.id === franchise.id ? 'text-teal-600' : 'text-slate-900'
                                      }`}>{franchise.name}</h3>
                        ...
                        <div className="flex p-1.5 bg-slate-200/50 rounded-2xl gap-1">
                          {categories.map((cat) => (
                            <button
                              ...
                              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm transition-all ${activeCategory === cat.id
                              ...
                              >
                                ...
                                {inCart ? (
                                  ...
                                  <button
                                    ...
                                    className={`w-full py-3 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${product.category === 'Raw Material' && inventory.find(i => i.id === product.id)?.quantity === 0
                            ...
                            {cart.length === 0 ? (
                              ...
                              <>
                                ...
                                <div className="pt-6 border-t border-slate-100 space-y-4">
                                  ...
                                  <button
                                    onClick={() => isOrdersOnline && setStep(3)}
                                    disabled={!isOrdersOnline}
                                    className={`w-full py-4 rounded-2xl font-bold text-lg transition-all flex items-center justify-center gap-2 ${isOrdersOnline
                                      ? 'bg-teal-600 text-white hover:bg-teal-700 shadow-lg shadow-teal-200'
                                      : 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none'
                                      }`}
                                  >
                                    {isOrdersOnline ? 'Review Order' : 'Ordering Disabled'}
                      ...
                      <div key={idx} className="flex flex-col gap-2 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <p className="font-black text-slate-900">Rs. {((item.price * item.quantity) - (item.itemDiscount || 0)).toLocaleString()}</p>
                            {item.itemDiscount ? (
                              <p className="text-[10px] text-green-500 font-bold uppercase tracking-widest mt-0.5">Discount: -Rs. {item.itemDiscount.toLocaleString()}</p>
                            ) : null}
                          </div>
                        </div>
                      </div>
